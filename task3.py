...
def _euclidean_distance(point1: Optional[list], point2: Optional[list]) -> float:
    if point1 is None or point2 is None:
        return float('inf')
    return np.sqrt((point1[0] - point2[0]) ** 2 + (point1[1] - point2[1]) ** 2)


def _find_count_of_person(image: Any, boxes: Any, confidences: Any, class_ids: Any) -> Tuple[int, list, list]:
    num_people = 0
    people_centers = []
    people_boxes = []

    for i in range(len(boxes)):
        if int(class_ids[i]) == 0:  # Проверяем, что class_id равно 0 для людей
            num_people += 1
...
def find_person_on_image(image_path: str, result_path: str, model: str) -> None:
    ...
    if cv2.imwrite(result_path, output_image):
...
if __name__ == "__main__":
    ...
    result_path = r'C:\Users\ftox\PycharmProjects\OPenCV\result.png'
